...
*** local vars in =org-noter-insert-note=
    ...
    | *selected-text-is-short*     | bool    | selected text is short                         |                             |
...
*** refactor of insert-note
    ...
    2. check for [[file:org-noter-core.el::1907][selected-text]] and set selected-text-is short
       let (selected-text-is-short (<= (length selected-text) org-noter--max-short-length))
    ...
    5. if not title, generate maybe-title from template:
